...
plotclr = brewer.pal(nclr,"Greens") #using brewer for a palette of green 
# define the classes or breaks for the data
class = classIntervals(fsa.shp@data$Median_Fam, nclr, style="quantile", dataPrecision=0) #breaking the population into quantiles, so by distribution
...
plot(fsa.shp, col=colcode, add=T) #add to the shape file the colour according to the 5 classes
...
ili$rate = (ili$visits / 3) / ili$pop #divided to get an estimate of the average/annual rate but don't do this elsewhere 
...
for (group in groups) {
  ...
  plotclr = brewer.pal(nclr, 'Reds') #get red for the number of classes 
  class.crude = classIntervals((ili$pop), group, style='quantile', dataPrecision=0) #this is the classINT function 
  colcode.crude = findColours(class.crude, plotclr)
  
  plot(fsa.shp)
  plot(fsa.shp, col=colcode.crude, add=T)
  title(sub="Population of Districts")
  legend('topleft', legend=names(attr(colcode.crude, "table")), fill=attr(colcode.crude,"palette"), cex=0.9, bty='n')
} # for - levels

par(mfrow=c(1,1))

#Plot my own variable 
groups = c(3,5,7,9)
par(mfrow=c(2,2))
...
moran.plot(sqrt(ili.ebe$raw), fsa.w, labels=fsa.visits$fsa) #this gives the global Moran's I 
...
moranlocal <- cbind(ili.ebe$raw,ili.moran.local) #comapre local Moran I values to raw 
